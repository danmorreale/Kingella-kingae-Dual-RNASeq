...
#following block finds the averages of each condition across replicates
  ...
  LogFC_1hpi = log2(infected_1hpi.AVG - uninfected_1hpi.AVG),
  LogFC_8hpi = log2(infected_8hpi.AVG - uninfected_8hpi.AVG),
  LogFC_14hpi = log2(infected_14hpi.AVG - uninfected_14hpi.AVG))
...
myTopHits_hpi_1.df <- myTopHits_hpi_1 %>% # This will need to be adjusted to match the above list.)
  ...
  mutate(
    sig = if_else(abs(logFC) > 1 & adj.P.Val < 0.05, "significant", "ns")
  )
...
sel_plots <- ggplot() +
  ...
  geom_point(data=to_plot_means,
             ...
             size=4, shape=18) +
  labs(x = "Hours Post Infection", y = "log2(CPM)", color = "Condition") + 
